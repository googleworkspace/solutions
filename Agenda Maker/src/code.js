...
function checkFolder (){
  ...
  while(folders.hasNext()){
    ...
    if ((folder.getDescription() == "App Script App - Do not change this description") && (folder.getOwner().getEmail()==Session.getActiveUser().getEmail()))
    {
        return folder.getId();
    }
  }
...
function getTemplateId(folderId){
  ...
  body.appendParagraph( "##Attendees##").setHeading(DocumentApp.ParagraphHeading.HEADING1).editAsText().setBold(true);
  body.appendParagraph( " ").editAsText().setBold(false);

  body.appendParagraph( "Overview").setHeading(DocumentApp.ParagraphHeading.HEADING1).editAsText().setBold(true);
  ...
  body.appendParagraph( "Next Steps").setHeading(DocumentApp.ParagraphHeading.HEADING1).editAsText().setBold(true);
...
function onCalendarChange(){
  ...
  var now = new Date();
  var events = CalendarApp.getEvents(now, new Date(now.getTime() + (2 * 60 * 60 * 1000000)),{search: '#agenda'});
  ...
  for (i = 0; i < events.length; i++) {
    ...
    if (event.isOwnedByMe()){
      ...
      description = description.replace("##agenda", "<a href=" + agendaUrl + ">Agenda Doc</a>");
